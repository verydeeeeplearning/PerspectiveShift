...
describe("GenerateReflectionQuizUseCase", () => {
  ...
  it("generates quiz with 4 options", () => {
    const result = uc.execute({ dialogueId: "d-1", opponentKeyPoint: "AI가 일자리를 대체한다" });
    expect(result.options).toHaveLength(4);
    expect(result.correctIndex).toBeGreaterThanOrEqual(0);
    expect(result.correctIndex).toBeLessThan(4);
  });

  it("includes dialogue ID in result", () => {
    const result = uc.execute({ dialogueId: "d-99", opponentKeyPoint: "포인트" });
    expect(result.dialogueId).toBe("d-99");
  });

  it("correct option contains opponent key point", () => {
    const keyPoint = "기술 발전이 불평등을 심화";
    const result = uc.execute({ dialogueId: "d-1", opponentKeyPoint: keyPoint });
